...
- Под макросом WIFI_DEBUG_FIXES:
  - `WiFi.mode(WIFI_STA);`
  - `WiFi.setSleep(false);` // Отключает спящий режим WiFi
...
## 37. Сохранение WiFi конфигурации в LittleFS и улучшенное сканирование сетей

### Описание изменений
- **Сохранение WiFi учетных данных в flash-память:** Добавлена функциональность автоматического сохранения и загрузки SSID/пароля в LittleFS. При первом запуске используются значения по умолчанию из `fake_network_definitions.h` (SSID: "fake_ssid", пароль: "fake_pass"). После настройки через GUI учетные данные сохраняются и восстанавливаются при перезагрузке.
- **Улучшенное сканирование WiFi сетей:** Модифицирован код сканирования в `WiFiManager::connect()` для отображения детальной информации о сетях:
  - SSID (выровнено по левому краю, 20 символов)
  - Уровень сигнала RSSI (выровнено по правому краю, 3 цифры)
  - Номер канала (выровнено по правому краю, 2 цифры)
  - Тип аутентификации (Open, WEP, WPA, WPA2, WPA/WPA2, WPA2-Enterprise, WPA3, Unknown)

### Формат вывода логов сканирования
```
Network: MyHomeWiFi           RSSI: -45dBm Ch:  6 Auth: WPA2
Network: GuestNetwork         RSSI: -67dBm Ch: 11 Auth: WPA2
Network: OpenCafe             RSSI: -52dBm Ch:  1 Auth: Open
```

### Технические детали
- **Сохранение в LittleFS:** Файл `/wifi_credentials.txt` содержит SSID и пароль по отдельным строкам. Валидация: SSID не пустой, пароль >= 8 символов.
- **Загрузка из LittleFS:** При неудаче чтения или невалидных данных используются значения по умолчанию.
- **Сканирование:** Используется `WiFi.scanNetworks()` с последующей обработкой каждого результата через `WiFi.encryptionType(i)` для определения типа шифрования.
- **Включение сканирования:** Только при `WIFI_DEBUG_FIXES && WIFI_ENABLE` для избежания лишних операций.

### Преимущества
- **Удобство:** WiFi настройки сохраняются между перезагрузками, не нужно перенастраивать после прошивки.
- **Диагностика:** Детальная информация о доступных сетях помогает выбрать оптимальную сеть и канал.
- **Безопасность:** Шифрование и другие параметры сетей отображаются для оценки безопасности.

### Тестирование
Проект компилируется. WiFi учетные данные корректно сохраняются и восстанавливаются. Сканирование показывает полный набор информации о сетях в читаемом табличном формате.

## 38. Добавление поддержки HTTPS с настраиваемой верификацией сертификатов

### Описание изменений
- **Новый define USE_HTTPS 0** в `lib/lora_config.hpp` для включения HTTPS протокола
- **Новый define USE_INSECURE_HTTPS 1** в `fake_network_definitions.h` для контроля верификации SSL сертификатов (эквивалент curl -k)
- **Условная компиляция протоколов:** При `USE_HTTPS=1` используются "https" и порт 443 вместо "http" и 80
- **WiFiClientSecure с настраиваемой верификацией:** При `USE_HTTPS=1` применяется `WiFiClientSecure`, при `USE_INSECURE_HTTPS=1` вызывается `setInsecure()` для пропуска проверки сертификатов
- **Обновленные логи:** В `loadSettings()` добавлен блок "HTTPS settings:" с `USE_HTTPS` и `USE_INSECURE_HTTPS`
- **Логи после подключения:** В `connect()` добавлено "Server protocol: %s, Port: %s" для отображения выбранной конфигурации
- **CURL примеры с -k флагом:** При `USE_HTTPS && USE_INSECURE_HTTPS` примеры curl включают флаг `-k`
- **Русские комментарии:** Комментарий к `USE_INSECURE_HTTPS` переведен на русский язык

### Преимущества новой реализации
- **Полная поддержка HTTPS:** Возможность работы с защищенными серверами
- **Гибкая верификация:** Можно отключить проверку сертификатов для тестирования (curl -k эквивалент)
- **Настраиваемые порты:** Автоматическое переключение на 443 для HTTPS
- **Подробные логи:** Информация о протоколе и настройках отображается в логах при загрузке и подключении
- **Обратная совместимость:** По умолчанию HTTP, HTTPS включается define'ом

### Технические детали
- **Протоколы:** Условно определяются в `fake_network_definitions.h` на основе `USE_HTTPS`
- **Порты:** PHP сервер использует 80/443, Flask сервер использует 5001/443
- **Клиент:** `WiFiClient` для HTTP, `WiFiClientSecure` для HTTPS с опциональным `setInsecure()`
- **Логирование:** Дополнительная информация в `loadSettings()` и `connect()` для диагностики

### Сценарии использования
1. **HTTP (дефолт):** `USE_HTTPS=0` - обычные HTTP запросы
2. **HTTPS с верификацией:** `USE_HTTPS=1, USE_INSECURE_HTTPS=0` - полная безопасность
3. **HTTPS без верификации:** `USE_HTTPS=1, USE_INSECURE_HTTPS=1` - для тестирования с самоподписанными сертификатами

### Тестирование
Проект компилируется. При `USE_HTTPS=1` используются HTTPS протокол и порт 443. Логи показывают выбранную конфигурацию. CURL примеры включают `-k` при отключенной верификации сертификатов.

## 39. Поддержка nip.io для публичного доступа к HTTPS серверам

### Описание изменений
- **Функция getNipIoUrl():** Добавлена новая функция в `WiFiManager` для преобразования IP адресов в формат nip.io
- **Преобразование IP:** IP адрес вида `192.168.1.1` преобразуется в `192-168-1-1.nip.io`
- **CURL примеры с nip.io:** При `USE_HTTPS=1` curl примеры используют nip.io URL для публичного доступа
- **Примеры с -k:** При `USE_INSECURE_HTTPS=1` добавляется флаг `-k` в curl примеры

### Преимущества новой реализации
- **Публичный доступ:** nip.io позволяет обращаться к локальным серверам по HTTPS через публичные DNS имена
- **Удобные curl примеры:** Пользователи могут копировать готовые команды для тестирования API
- **Динамическая генерация:** URL генерируется автоматически на основе настроенного IP адреса сервера

### Технические детали
- **nip.io сервис:** Бесплатный wildcard DNS сервис, где `xxx.nip.io` разрешается в IP `xxx` (с заменой дефисов на точки)
- **Формат URL:** `https://192-168-1-1.nip.io:443/api/lora` для IP `192.168.1.1`
- **Логика генерации:** Функция `getNipIoUrl()` заменяет точки на дефисы и добавляет `.nip.io`

### Сценарии использования
1. **Локальная разработка:** Доступ к локальному HTTPS серверу через публичный URL
2. **Тестирование API:** Прямое копирование curl команд из логов для тестирования
3. **Удаленный доступ:** Возможность обращаться к устройству через интернет без статического IP

### Примеры curl команд в логах
```
CURL example: curl -k -X POST -H 'Content-Type: application/json' -d '{"user_id": "test"}' https://192-168-1-1.nip.io:443/api/lora
```

### Тестирование
Функция корректно преобразует IP адреса. При `USE_HTTPS=1` curl примеры используют nip.io формат, что позволяет тестировать API через публичные URL.

## 40. Повышение надежности доставки LoRa данных с повторными попытками и статистикой

### Описание изменений
- **Механизм повторных попыток:** При неудачной отправке POST запроса на сервер, запрос автоматически возвращается в начало очереди для повторной отправки. Предотвращает потерю данных при временных проблемах сети.
- **Статистические счетчики:** Добавлены два новых счетчика в класс `WiFiManager`:
  - `loraPacketsReceived` - общее количество принятых LoRa пакетов
  - `postRequestsSent` - количество успешно отправленных POST запросов
- **Использование счетчиков в POST данных:** Поля `cold` и `hot` теперь содержат актуальную статистику вместо счетчиков:
  - `cold`: количество принятых LoRa пакетов (loraPacketsReceived)
  - `hot`: количество успешных POST отправок (postRequestsSent)
- **Логирование счетчиков:** При каждой успешной отправке выводится: `"Queued POST success - Total sent: X, received: Y"`
- **Обновление Flask сервера:** API теперь поддерживает обработку батчей (массивов JSON объектов) и помечает их `additional_field4 = 1`, одиночные запросы получают `additional_field4 = 0`

### Технические детали
- **Повторные попытки:** В `doHttpPostFromData()` при HTTP коде ≠ 200 запрос возвращается в очередь через `postQueue.insert(postQueue.begin(), postData)`
- **Счетчики:** `volatile unsigned long` переменные с getter методами для потокобезопасности
- **Инкремент счетчиков:** `loraPacketsReceived++` при приеме LoRa пакета, `postRequestsSent++` при успешной отправке
- **Батч обработка:** Flask сервер обрабатывает массивы объектов, устанавливая `additional_field4 = 1` для батчей и `0` для одиночных
- **Статистика в полях:** `cold` и `hot` заменяют старые счетчики на актуальную статистику работы устройства

### Преимущества новой реализации
- **Надежность:** Данные не теряются при временных сбоях сети
- **Мониторинг:** Статистика работы устройства в каждом POST запросе
- **Масштабируемость:** Поддержка батчей для снижения нагрузки при высоком трафике
- **Обратная совместимость:** Сохраняются все существующие поля и логика

### Сценарии использования
1. **Нормальная работа:** Счетчики растут последовательно, данные доставляются успешно
2. **Сбои сети:** При временных проблемах данные сохраняются в очереди для повторной отправки
3. **Высокий трафик:** Батчи автоматически собираются и отправляются (BATCH_SIZE=5)
4. **Мониторинг:** Сервер получает статистику работы каждого устройства

### Тестирование
Проект компилируется. Прием LoRa пакетов корректно учитывается в счетчиках. POST запросы содержат актуальную статистику. При отключении сервера данные остаются в очереди. Flask сервер корректно обрабатывает как одиночные запросы, так и батчи с соответствующей пометкой additional_field4.
